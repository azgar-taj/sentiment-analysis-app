...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link entry_placeholder} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<entry_placeholder> extractEarthquakes(String x) {
        SAMPLE_JSON_RESPONSE = x;
        if(x.equals("")){
            ArrayList<entry_placeholder> ar = new ArrayList<>();
            return ar;
        }
        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<entry_placeholder> earthquakes = new ArrayList<>();

        // Try to parse the SAMPLE_JSON_RESPONSE. If there's a problem with the way the JSON
        // is formatted, a JSONException exception object will be thrown.
        // Catch the exception so the app doesn't crash, and print the error message to the logs.
        try {
            JSONObject start = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray features = start.getJSONArray("features");

            for(int i = 0; i < features.length();i++){
                JSONObject feature = features.getJSONObject(i);
                JSONObject props = feature.getJSONObject("properties");
                String index = props.getString("mag");
                String place = props.getString("place");
                String time = Long.toString(props.getLong("time"));
                String web = props.getString("url");
                earthquakes.add(new entry_placeholder(index,place,time,web));
            }
